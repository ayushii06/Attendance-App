...
function FaceRegistration({ isFaceRegistered, user_id }) {
  const [createFaceEnrollment, { isLoading }] =
  ...
  const [status, setStatus] = useState(
    isFaceRegistered ? "registered" : "notRegistered"
  );
  ...
  const startCaptureProcess = async () => {
    ...
    try {
      const result = await createFaceEnrollment({
      ...
      setStatus("success");
      toast.success("Face registration successful! Please login again.");
      ...
      setErrorMessage(
        err.data?.message || "Failed to enroll face. Please try again."
      );
      stopWebcam();
        ...
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            <div className="bg-amber-100 p-4 rounded-full mb-4">
              <FiUserX className="h-10 w-10 text-amber-500" />
            </div>
            ...
            <button
              ...
              className="mt-8 inline-flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-colors duration-200"
            >
              <FiCamera className="h-5 w-5 mr-2" /> Register Your Face
            </button>
          </div>
        );
      case "registered":
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            ...
            <button
              onClick={() => setStatus("retakeInfo")}
              className="mt-8 inline-flex items-center justify-center px-5 py-2.5 bg-slate-100 text-slate-700 font-semibold rounded-lg border border-slate-300 hover:bg-slate-200 transition-colors duration-200"
            >
        ...
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            <div className="bg-sky-100 p-4 rounded-full mb-4">
              <ShieldAlert className="h-10 w-10 text-sky-600" />
            </div>
            <h2 className="text-2xl font-bold text-slate-800">
              Admin Permission Required
            </h2>
            <p className="text-slate-500 mt-2 max-w-md">
              Please contact your system administrator to request a photo
              update.
            </p>
            <button
              ...
              className="mt-8 inline-flex items-center justify-center px-6 py-3 bg-indigo-600 text-white font-semibold rounded-lg shadow-md hover:bg-indigo-700 transition-colors duration-200"
            >
              <FiArrowLeft className="h-5 w-5 mr-2" /> Go Back
            </button>
          </div>
        );
      case "capturing":
          ...
          <div className="flex flex-col items-center justify-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            ...
            <div className="relative w-full max-w-lg aspect-video bg-slate-900 rounded-lg overflow-hidden border-2 border-slate-300 shadow-lg">
              ...
              <div className="absolute inset-0 bg-black bg-opacity-10 flex items-center justify-center">
                <div className="w-3/4 h-5/6 border-4 border-dashed border-white/50 rounded-3xl"></div>
              </div>
            </div>
              ...
              <div className="mt-4 flex justify-center space-x-2 h-16">
                {capturedImages.map((src, index) => (
                  <img
                    key={index}
                    src={src}
                    className="h-16 w-16 object-cover rounded-md border-2 border-indigo-300 shadow-sm"
                    alt={`Capture ${index + 1}`}
                  />
                ))}
              </div>
      ...
      case "submitting":
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            ...
            <p className="text-slate-500 mt-1">
              Sending photos for verification. Please hold on.
            </p>
          </div>
        );
      case "success":
      ...
      case "error":
        return (
          <div className="flex flex-col items-center justify-center text-center p-8 bg-white rounded-xl shadow-sm border border-slate-200 h-full">
            ...
            <button
              onClick={startCaptureProcess}
              className="mt-8 inline-flex items-center justify-center px-6 py-3 bg-red-600 text-white font-semibold rounded-lg shadow-md hover:bg-red-700 transition-colors duration-200"
            >
