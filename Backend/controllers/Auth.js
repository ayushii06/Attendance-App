  ...
  try {
    ...
    if (activeOTP) {
      const timeSinceLastOTP =
        (Date.now() - new Date(activeOTP.createdAt).getTime()) / 1000;
...
exports.signUp = async (req, res) => {
  try {
    ...
    if (accountType === "Student") {
      ...
      if (!branchDetails) {
        return res
          ...
          .json({
            success: false,
            message: "Invalid branch or Branch not Updated yet",
          });
      }
    ...
    res
      .status(500)
      .json({ success: false, message: "Error signing up user", error: error });
  }
};
...
exports.login = async (req, res) => {
  try {
    ...
    let user = await User.findOne({ email })
      .populate("courses", "courseName year")
